...
export function Dashboard({ session }: DashboardProps) {
  ...
  async function handleRedeemPromo() {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Promo code could not be applied.");
      }

      if (typeof json?.newBalance === "number") {
        setCreditsBalance(json.newBalance);
      }
      ...
      if (json?.ledgerEntry) {
        const entry: AgentCreditLedgerEntry = {
          ...
          listingId:
            (json.ledgerEntry.listingId as string | null | undefined) ?? null,
      ...
      setPromoStatus(
        added > 0
          ? `Promo applied: +${added} credit${added === 1 ? "" : "s"}`
          : "Promo applied.",
      );
      setPromoCode("");
    } catch (err: any) {
        ...
        <div className="grid gap-4 md:grid-cols-[1.4fr,1fr]">
          ...
          <div className="space-y-3">
            <div className="space-y-2 rounded-md border border-dashed border-zinc-300 bg-zinc-50 px-3 py-2">
              ...
              <p className="text-[10px] text-zinc-500">
                Enter a promo code to add free listing credits to your
                balance.
              </p>
              ...
              {promoError && (
                <div className="text-[10px] text-red-700">{promoError}</div>
              )}
            </div>
